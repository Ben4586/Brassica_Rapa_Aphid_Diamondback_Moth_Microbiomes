...
seqtab2 <- seqtab[,nchar(colnames(seqtab)) %in% seq(370,430)] #remove seqs that are too long to be correct.  This parameter can be adjusted as needed. 
...
mapfile <- DBM_metadata.csv" #make a csv file with sample metadata included in data folder. 
...
ps_filtered <- subset_taxa(ps_rarefied, !is.na(Phylum) & !Phylum %in% c("Abditibacteriota", "Deferribacterota", "Methylomirabilota", "Deinococcota", "Myxococcota", "Spirochaetota", "Armatimonadota", "Desulfobacterota", "Nitrospirota", "Patescibacteria"))
...
# Merge sequence object into the phyloseq object: 470 taxa in 12 samples
